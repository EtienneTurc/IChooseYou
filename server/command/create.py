...
label_help += " Hey ! <user> choose <element> to <your_label>\n"
...
pick_list_help += " If you want to directly notify a user when he is selected,"
...
class CreateCommand(BaseCommand):
    def __init__(self, text, channel_id):
        ...
        name = "create"
        args = [
            Arg(name="name", nargs=1, help="Name of the command to create."),
            Arg(name="label", nargs="+", help=label_help),
            Arg(name="pickList", nargs="+", type=list, help=pick_list_help),
            Arg(
                name="selfExclude",
                nargs="?",
                default=False,
    ...
    def exec(self):
        ...
        created_command = Command.find_one_by_name_and_chanel(
            self.options["name"], self.channel_id
        )
